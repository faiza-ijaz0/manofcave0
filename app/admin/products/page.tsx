...
export default function SuperAdminProducts() {
    ...
    const fetchProducts = async () => {
      try {
        ...
        if (user?.role === 'super_admin') {
          // Super admin - sab products dekhe
          q = query(productsRef, orderBy('createdAt', 'desc'));
          console.log('👑 Super Admin: All products');
        } else {
          // Default - all products (client side filter lagayenge)
          q = query(productsRef, orderBy('createdAt', 'desc'));
        }
  ...
  const branchProducts = user?.role === 'admin' && user?.branchId 
    ? products.filter(p => p.branches.includes(user.branchId!))
    : products;

  const totalProducts = branchProducts.length;
  const activeProducts = branchProducts.filter(p => p.status === 'active' || p.status === 'low-stock').length;
  const totalStockValue = branchProducts.reduce((acc, product) => acc + (product.price * product.totalStock), 0);
  const totalRevenue = branchProducts.reduce((acc, product) => acc + product.revenue, 0);
  const lowStockItems = branchProducts.filter(p => p.status === 'low-stock' || p.status === 'out-of-stock').length;
  const avgMargin = branchProducts.length > 0 
    ? branchProducts.reduce((acc, product) => acc + ((product.price - product.cost) / product.price * 100), 0) / branchProducts.length 
    : 0;
                ...
                <div>
                  ...
                  <p className="text-sm text-gray-600">
                    {user?.role === 'super_admin' 
                      ? "Manage retail inventory across all branches" 
                      : `Managing products for ${user?.branchName || 'your branch'}`
                    }
                  </p>
                ...
                <div className="text-center py-12">
                  ...
                  <p className="text-gray-600 mb-4">
                    {searchTerm || categoryFilter !== 'all' || priceFilter !== 'all' || stockFilter !== 'all'
                      ...
                      : `No products available for ${user?.branchName || 'your branch'}`
                    }
                  </p>
                  {!searchTerm && categoryFilter === 'all' && priceFilter === 'all' && stockFilter === 'all' && (
